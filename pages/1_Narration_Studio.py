...
with st.sidebar:
    ...
    if st.session_state.get("api_key"):
        ...
        if st.button("🔄 목소리 목록 새로고침", use_container_width=True):
            ...
            if isinstance(updated, dict) and "error" not in updated:
                ...
                st.error(updated.get("error", "알 수 없는 오류"))
    else:
        st.error("❌ API Key 없음")
        st.caption("메인 페이지 사이드바에서 ElevenLabs API Key를 먼저 설정하세요.")
...
if not st.session_state.get("api_key"):
    st.warning("⚠️ ElevenLabs API Key가 설정되지 않았습니다. 메인 페이지 사이드바에서 먼저 설정해 주세요.")
    st.stop()
...
with st.expander("👥 화자 설정 — 등장인물별 목소리 배정", expanded=True):
    st.caption("나레이션에 등장하는 화자를 추가하고 각각에 ElevenLabs 목소리를 배정하세요.")
...
# 컬럼 헤더
gh1, gh2, gh3, gh4, gh5 = st.columns([5, 2, 2, 2, 0.5])
gh1.caption("문장")
gh2.caption("화자")
gh3.caption("감정")
gh4.caption("속도")
...
with c1:
    if st.button("✨ 전체 생성", type="primary", use_container_width=True):
        segs_to_gen = [
...
with c2:
    ...
    if gen_sids:
        if st.button("🔗 전체 합치기", use_container_width=True):
        ...
        st.button("🔗 전체 합치기", use_container_width=True, disabled=True)
...
with c3:
    if st.button("🔄 초기화", use_container_width=True, help="모든 문장과 음원을 초기화합니다"):
        prefix_list = (
            "narr_seg_text_", "narr_seg_sp_", "narr_seg_mood_", "narr_seg_speed_",
            "narr_sp_name_", "narr_sp_voice_", "narr_gen_", "narr_del_", "narr_dl_",
        )
